...
class MPC:
	...
	def __init__(self, model, N, T, Q_0, R_0, eq_constraint, ineq_constraint, term_ineq_constraint, term_eq_constraint, code_gen_file_name):
		...
		for i in range(1, self.N+1):
			...
			for j in range(0, self.model.NU):
				control_vec[j, 0] = self.sym_control_vector[j,i-1]
				control_ref_vec[j, 0] = self.sym_control_ref_vector[j,i-1]
			self.sym_X = self.sym_X.col_join(control_vec)
			self.sym_X_ref = self.sym_X_ref.col_join(control_ref_vec)
			
			if(i < N):
				add = (state_vec-state_ref_vec).T*Q_0[0:model.NX, 0:model.NX]*(state_vec-state_ref_vec)
				add += (control_vec-control_ref_vec).T*Q_0[model.NX:, model.NX:]*(control_vec-control_ref_vec)
				self.J += add
			else:
				add = state_vec.T*R_0[0:model.NX, 0:model.NX]*state_vec
				self.J += add
		self.J *= 0.5
		...
		self.q = self.J.jacobian(self.sym_X)
	...
	def initialiseEqualityConstraints(self):
		...
		self.h = []
		temp_f = self.model.getForwardIntegration()

		for j in range(0, self.model.NX):
			temp_f = temp_f.subs(self.model.state[j], self.x0[j])
			temp_f[j,0] = -temp_f[j,0] + self.sym_state_vector[j,1]
		for j in range(0, self.model.NU):
			temp_f = temp_f.subs(self.model.control[j], self.sym_control_vector[j,0])
		for j in range(0, self.model.NP):
			temp_f = temp_f.subs(self.model.params[j], self.params[j,0])
		self.h.append(expand(temp_f))
		...
		if(self.term_eq_constraint != None):
			self.h.col_join(self.term_eq_constraint(self.sym_state_vector, self.params))
		# discretisation time
	...
	def initialiseInequalityConstraints(self):
		...
		if(self.ineq_constraint != None):
			self.g = self.g.col_join(self.ineq_constraint(self.sym_state_vector, self.sym_control_vector , self.params))
		if(self.term_ineq_constraint != None):
			self.g = self.g.col_join(self.term_ineq_constraint(self.sym_state_vector, self.sym_control_vector, self.params))
